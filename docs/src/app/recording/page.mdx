...
The default `cursor` preset renders a synthetic cursor into the video because CDP screenshots do not include the OS cursor. Effects are injected into the recorded page, so Chromium renders cursor SVGs, click bursts, overlays, and zoom animations before each frame is captured. Clicks show cursor flight plus click ripple only. Use `--record-mode demo` for slower presentation cursor timing with the cursor visible between actions, then add zoom and overlay emphasis explicitly.
