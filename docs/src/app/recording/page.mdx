...
The default `cursor` preset renders a synthetic cursor into the video because CDP screenshots do not include the OS cursor. Clicks show cursor flight plus click ripple only. Use `--record-mode demo` for presentation timing defaults, then add zoom and overlay emphasis explicitly.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>demo</code></td><td>Cursor and click pulse with blocking click timing and animated fill/type defaults</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>record overlay text &lt;text&gt;</code></td><td>Add a serialized subtitle-style text overlay to the recording. Defaults to five seconds unless <code>--duration-ms</code> is passed.</td></tr>
    <tr><td><code>record overlay spotlight &lt;selector|ref&gt;</code></td><td>Highlight an element in the recording</td></tr>
    <tr><td><code>record zoom to &lt;selector|ref&gt;</code></td><td>Apply camera zoom around an element without changing page layout. Holds until reset unless <code>--duration-ms</code> is passed.</td></tr>
