...
agent-browser record start <path>             # Start video recording (WebM); cursor on by default
agent-browser record start <path> --no-cursor # Record without the cursor overlay
agent-browser record start <path> --cursor dot # Override cursor theme (arrow|dot|hand)
