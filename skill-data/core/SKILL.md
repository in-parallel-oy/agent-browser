...
A synthetic animated cursor is baked into the recording **by default** (the OS cursor isn't captured because frames come from `Page.captureScreenshot`). The default cursor preset renders effects in the recorded page. Demo mode records a stable compositor surface so cursor, overlay, spotlight, and zoom animations remain independent of page navigation. Use `--record-mode demo` for human-readable timing defaults: cursor flight and click pulses are slower, the cursor stays visible between actions, clicks wait for the cursor tween, fill/type actions animate with a short delay, presentation overlay commands hold for their visible duration, zoom commands wait for the camera transition, and recording automatically pauses between actions so agent inference delays do not create idle video. Clicks do not zoom automatically; add emphasis with explicit `record overlay` and `record zoom` commands. Override the cursor theme with `--cursor <theme>` (`arrow`, `dot`, `hand`) or disable effects entirely with `--record-effects off`:
